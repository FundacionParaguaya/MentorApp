...
function SyncOffline(props) {
  return (
    <View style={styles.view}>
      <Text style={globalStyles.h3}>{i18n.t('views.sync.offline')}</Text>
      <Icon style={styles.icon} name="wifi-off" size={60} color={colors.grey} />
      <Text style={globalStyles.p}>
        {props.pendingDraftsLength === 1
          ? i18n.t('views.sync.updatePending')
          : i18n
              .t('views.sync.updatesPending')
              .replace('%n', props.pendingDraftsLength)}
      </Text>
    </View>
  );
}
