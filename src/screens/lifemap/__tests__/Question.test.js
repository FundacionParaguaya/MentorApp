...
it('shows question definition popup when prompted', () => {
  ...
  wrapper.find('#show-definition').props().onPress();
...
it('updates draft on selecting a question', () => {
  ...
  wrapper.find(SliderComponent).props().selectAnswer(3);
  ...
  wrapper.find(SliderComponent).props().selectAnswer(2);
...
it('updates draft on skipping a question', () => {
  wrapper.find(IconButton).props().onPress();
...
it('navigates to skipped questions screen upon answering last question when there are such', () => {
  props = createTestProps({
    drafts: [{...draft, indicatorSurveyDataList: [{key: 'income', value: 0}]}],
    navigation: {
      ...navigation,
      getParam: jest.fn((param) => {
        if (param === 'survey') {
