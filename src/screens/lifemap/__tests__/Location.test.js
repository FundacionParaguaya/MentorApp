...
describe('user is online', () => {
  ...
  it('allows user to serch for location', () => {
    ...
    searchBar.props().onPress({}, {geometry: {location: {lat: 4, lng: -1}}});
...
describe('user is offline', () => {
  ...
  it('checks if user is whichn cached map bounds', () => {
    ...
    expect(
      wrapper.instance().isUserLocationWithinMapPackBounds(4, 4, [
        [
          [1, 1],
          [2, 3],
        ],
...
describe('user offline with no cached maps', () => {
  ...
  it('allows user to edit each field on offline form', () => {
    wrapper.find('#address').props().onChangeText();
    wrapper.find('#postCode').props().onChangeText();

    wrapper.find('#country').props().onChange();
  ...
  it('sets errors on each field change', () => {
    ...
    wrapper.find('#address').props().setError(true);

    expect(spy).toHaveBeenCalledWith(true, 'address');

    wrapper.find('#postCode').props().setError(true);

    expect(wrapper).toHaveState({errors: ['address', 'postCode']});

    wrapper.find('#postCode').props().setError(false);

    expect(wrapper).toHaveState({errors: ['address']});

    wrapper.find('#country').props().setError(true);
  });
